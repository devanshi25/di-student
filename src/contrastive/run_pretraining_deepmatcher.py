...
class DataTrainingArguments:
    ...
    combo: Optional[bool] = field(
        default=None, metadata={"help": "Tp apply sequence combo augmentation"}
    )
...
def main():
    ...
    if training_args.do_train:
        ...
        if data_args.interm_file is not None:
            train_dataset = ContrastivePretrainDatasetDeepmatcher(train_dataset, tokenizer=model_args.tokenizer, intermediate_set=data_args.interm_file, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, aug_prob=data_args.augment_prob, combo=data_args.combo)
        else:
            train_dataset = ContrastivePretrainDatasetDeepmatcher(train_dataset, tokenizer=model_args.tokenizer, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, aug_prob=data_args.augment_prob, combo=data_args.combo)
        ...
        if training_args.load_best_model_at_end:
            ...
            test_dataset = ContrastivePretrainDatasetDeepmatcher(test_dataset, tokenizer=model_args.tokenizer, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, aug_prob=data_args.augment_prob, combo=data_args.combo)

    elif training_args.do_eval:
        if "validation" not in raw_datasets:
            raise ValueError("--do_eval requires a validation dataset")
        validation_dataset = raw_datasets["validation"]
        validation_dataset = ContrastivePretrainDatasetDeepmatcher(validation_dataset, tokenizer=model_args.tokenizer, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, aug_prob=data_args.augment_prob, combo=data_args.combo)
